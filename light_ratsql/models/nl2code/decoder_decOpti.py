...
class NL2CodeDecoder(torch.nn.Module):
    ...
    def __init__(
        ...
        #print('index of actions')
        #print(self.rules_index)
        #print('terminal voc')
        #print(self.terminal_vocab.elem_to_id)
        #print('size of rule_emb_size', rule_emb_size)
        ...
        if self.preproc.use_seq_elem_rules:
            ...
            self.node_type_vocab = vocab.Vocab(
                ...
                sorted(self.preproc.seq_lengths.keys()),
                special_elems=())
        #print(self.node_type_vocab.elem_to_id)
        #print(self.node_type_vocab.id_to_elem )


        self.state_update = variational_lstm.RecurrentDropoutLSTMCell(
            #input_size=self.rule_emb_size * 2 + self.node_emb_size + self.enc_recurrent_size * 3 + self.recurrent_size,
            input_size=self.node_emb_size*8,
            hidden_size=self.recurrent_size,
            dropout=dropout)

        self.syntax_role = torch.nn.Sequential(
            torch.nn.Linear(self.enc_recurrent_size, self.rule_emb_size),
            torch.nn.Tanh(),
            torch.nn.Linear(self.rule_emb_size, self.node_emb_size))

        self.previous_state = torch.nn.Sequential(
            torch.nn.Linear(self.rule_emb_size, self.rule_emb_size),
            torch.nn.Tanh(),
            torch.nn.Linear(self.rule_emb_size, self.node_emb_size))

        self.parent_state = torch.nn.Sequential(
        ...
        #print('shape', desc_enc.memory.shape)
        #self.global_context = self.syntax_role(desc_enc.memory).mean(dim=0, keepdim=True) 

        self.state__attention = attention.MultiHeadedAttention(h=8, query_size=self.node_emb_size, value_size=self.node_emb_size, final_size = self.node_emb_size)
    ...
    def compute_loss(self, enc_input, example, desc_enc, debug):
        if not (self.enumerate_order and self.training):
            ...
            #print('shape', desc_enc.memory.squeeze(0).shape)
            self.global_context = self.syntax_role(desc_enc.memory.squeeze(0)).mean(dim=0, keepdim=True)

            #print('shape', self.global_context.shape)
            
            mle_loss = self.compute_mle_loss(enc_input, example, desc_enc, debug)
            #self.sr_vector = self.syntax_role(desc_enc.others_memory)

            #print('it is me', example.tree)
        else:
            print('test2')
    ...
    def compute_mle_loss(self, enc_input, example, desc_enc, debug=False):
        ...
        #print(desc_enc.orth_loss)
        loss = torch.sum(torch.stack(tuple(traversal.loss), dim=0), dim=0) #+ desc_enc.orth_loss
    ...
    def begin_inference(self, desc_enc, example):
        #self.global_context = self.syntax_role(desc_enc.memory)
        self.global_context = self.syntax_role(desc_enc.memory.squeeze(0)).mean(dim=0, keepdim=True)
    ...
    def _desc_attention(self, prev_state, desc_enc):
        ...
        query = prev_state#[0]
        #memory_col, memory_tab = desc_enc.others_memory

        #ctx1, _ = self.desc_attn(query, memory_col, attn_mask=None)
        #ctx2, _ = self.desc_attn(query, memory_tab, attn_mask=None)
        #ctx3, _ = self.desc_attn(query, relation_schema, attn_mask=None)

        #ctx = ctx1 + ctx2 

        if self.attn_type != 'sep':
            #ctx4, att4 = self.desc_attn(query, desc_enc.memory, attn_mask=None)
            #ctx = (ctx + ctx4) / 3
            #return ctx, att4
    ...
    def _update_state(
        ...
        #previous = self.previous_state(torch.cat((prev_state[0], parent_h), dim=-1))
        
        short_context, _  = self._desc_attention(prev_state[0], desc_enc)
        long_context,  _ = self._desc_attention(prev_state[1], desc_enc)
        parent_context, _ = self._desc_attention(parent_h, desc_enc)


        #print(previous.shape)
        #print(prev_state[1].shape)

        #context = self.previous_state(torch.cat((prev_state[1], self.global_context), dim=-1))
    

        #struct_context, struct_attention_logits = self._struct_attention(prev_state[0], desc_enc.others_memory)
        #sr_vector = self.syntax_role(desc_enc.others_memory)
        ...
        s_input = torch.cat(
            (
                self.previous_state(prev_action_emb),  # a_{t-1}: rule_emb_size
                self.previous_state(parent_action_emb),

                self.parent_state(parent_h),  # s_{p_t}: recurrent_size

                node_type_emb,  # n_{f-t}: node_emb_size
                
                short_context,  # c_t: enc_recurrent_size
                long_context,
                parent_context,
            ...
            dim=0)
        #print(s_input.shape)
        state_input, attention_logits = self.state__attention(s_input, s_input)
        state_input = state_input.view(1, 512)
        #print(s_input.shape)

        # state_input shape: batch (=1) x (emb_size * 7)
