...
def compute_pointer_with_align(
    ...
    if node_type == "column":
        #print(desc_enc.m2c_align_mat.shape)
        pointer_probs = torch.mm(memory_pointer_probs, desc_enc.m2c_align_mat) #+  1.25e-3*torch.mm(memory_pointer_probs, desc_enc.pos2c)
    else:
        assert node_type == "table"
        pointer_probs = torch.mm(memory_pointer_probs, desc_enc.m2t_align_mat) #+  1.25e-3*torch.mm(memory_pointer_probs, desc_enc.pos2t)
