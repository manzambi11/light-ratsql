...
def compute_cell_value_linking(tokens, schema):
    ...
    return cv_link
#### Nathan Adding: Only not stop word token a
def compute_syntax_dependancy(question_list):
    ...
    #"question": ["how", "many", "head", "of", "the", "department", "be", "older", "than", "56", "?"]
    df=dict()
    question_str = " ".join(question_list)
    doc = nlp(question_str)
    #doc = [preprocess_token(token) for token in doc[i].children if is_token_allowed(token)]

    for i, token in enumerate(question_list):
            for j,child in enumerate([preprocess_token(token) for token in doc[i].children if is_token_allowed(token)]): 
                try:
                    df[f"{i},{question_list.index(str(child))}"] = "forward"
                    df[f"{question_list.index(str(child))},{i}"] = "backward"
                except:
                    pass
