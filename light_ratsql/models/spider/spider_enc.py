...
class SpiderEncoderV2Preproc(abstract_preproc.AbstractPreproc):
    ...
    def preprocess_item(self, item, validation_info):
        ...
        # Nathan Adding
        syn_dep=compute_syntax_dependancy(question)
        ...
        return {
            ...
            'syn_dep':syn_dep, #Nathan Adding S2 SQL
        }
...
class SpiderEncoderV2(torch.nn.Module):
    ...
    def __init__(
            ...
            device,
            preproc,
            word_emb_size=128, #128,
            recurrent_size=256,
            dropout=0.,
            question_encoder=('emb', 'bilstm'),
            column_encoder=('emb', 'linear'),
            table_encoder=('emb', 'linear'),
            update_config={},
    ...
    def forward_unbatched(self, desc):
        ...
        q_enc_new, c_enc_new, t_enc_new, others_memory = self.encs_update(desc, q_enc, c_enc, c_boundaries, t_enc, t_boundaries)
    ...
    def forward(self, descs):
        ...
        if self.batch_encs_update:
            q_enc_new, c_enc_new, t_enc_new, others_memory = self.encs_update(
        ...
        for batch_idx, desc in enumerate(descs):
            if self.batch_encs_update:
                ...
                q_enc_new_item, c_enc_new_item, t_enc_new_item, align_mat_item, others_memory = \
            ...
            result.append(SpiderEncoderState(
                ...
                others_memory = others_memory
                #pos2c = align_mat_item[2],
                #pos2t = align_mat_item[3],
                #orth_loss=lr
            ))
        return result
...
class SpiderEncoderBert(torch.nn.Module):
    ...
    def forward(self, descs):
        ...
        for batch_idx, desc in enumerate(descs):
            ...
            q_enc_new_item, c_enc_new_item, t_enc_new_item, align_mat_item, lr = \
                self.encs_update.forward_unbatched(
                    desc,
                    q_enc.unsqueeze(1),
                    col_enc.unsqueeze(1),
