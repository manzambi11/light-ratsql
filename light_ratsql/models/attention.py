...
class MultiHeadedAttention(torch.nn.Module):
    def __init__(self, h, query_size, value_size,final_size, dropout=0.1):
        super().__init__()
        ...
        self.linears = torch.nn.ModuleList([
            ...
            torch.nn.Linear(value_size, value_size),
            torch.nn.Linear(value_size, final_size),
        ])
...
class MultiHeadedAttention_(torch.nn.Module):
    def __init__(self, h, query_size, value_size, dropout=0.1):
        ...
        self.linears = torch.nn.Linear(query_size, value_size)
        self.f_linears = torch.nn.Linear(value_size, value_size)
    ...
    def forward(self, query, values, attn_mask=None):
        ...
        # 1) Do all the linear projections in batch from d_model => h x d_k
        query =self.linears(query).view(nbatches, -1, self.h, self.d_k).transpose(1, 2)
        values = values.view(8, -1, self.d_k)
        keys = values
        ...
        x = x.squeeze(1)
        return self.f_linears(x), self.attn
